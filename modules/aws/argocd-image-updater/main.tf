...
# Configures EventBridge to send ECR push events to ArgoCD Image Updater webhook
# Based on: https://github.com/argoproj-labs/argocd-image-updater/blob/master/config/examples/cloudevents/terraform/main.tf

# EventBridge Rule to capture ECR push events
resource "aws_cloudwatch_event_rule" "ecr_push" {
  name        = var.rule_name != "" ? var.rule_name : "argocd-image-updater-ecr-push"
...
resource "aws_cloudwatch_event_connection" "webhook" {
  name               = var.connection_name != "" ? var.connection_name : "argocd-image-updater-webhook"
  description        = "Connection to ArgoCD Image Updater webhook"
  authorization_type = "API_KEY"

  auth_parameters {
    api_key {
      key   = var.api_key_header_name
...
resource "aws_cloudwatch_event_api_destination" "webhook" {
  name                             = var.api_destination_name != "" ? var.api_destination_name : "argocd-image-updater-webhook"
...
resource "aws_iam_role" "eventbridge" {
  name               = var.iam_role_name != "" ? var.iam_role_name : "argocd-image-updater-eventbridge-role"
...
resource "aws_cloudwatch_event_target" "api_destination" {
  ...
  target_id = var.target_id != "" ? var.target_id : "ArgocdImageUpdaterCloudEvent"
